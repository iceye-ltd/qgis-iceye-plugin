...
def _show_slc(
    ...
    # `im` is discarded on purpose — no colorbar is drawn anywhere in the pipeline (kept lean by design).
    del im
...
def _show_phase_derivative(ax: plt.Axes, d: np.ndarray, title: str) -> None:
    ...
    ax.imshow(d, cmap="twilight", aspect="auto", vmin=-np.pi, vmax=np.pi)
    ax.set_title(title)
    ax.set_xlabel("range pixel")
    ax.set_ylabel("azimuth pixel")
...
def _show_detection_map(ax: plt.Axes, det: np.ndarray, title: str) -> None:
    ...
    ax.imshow(det, cmap="hot", aspect="auto", vmin=0, vmax=vmax)
    ax.set_title(title)
    ax.set_xlabel("range pixel")
    ax.set_ylabel("azimuth pixel")
...
def _af_find_best_deviation(
    ...
    *,
    ...
    for deviation in fine:
        c = deviation / x[-1] ** poly_degree
        fitted = -c * x ** poly_degree
        # Smooth the polynomial correction with a moving-average filter of length `N/10` (edge-padded boundaries so the parabola's non-zero endpoints are preserved). Softens the whole curve rather than only its centre; empirically dampens the outer-Doppler over-correction for moving targets where the true range-vs-Doppler is a shifted parabola instead of the symmetric `−c·f²` this search assumes.
        fitted = _mean_filter_1d(fitted, fitted.size // 10)
        shifted = np.abs(_af_shift_fitted(spatch_fft, fitted))
        C = _af_compute_contrast_subaperture_sums(shifted, N_subaperture)
        contrast_gain = _af_contrast_ratio_sum(C, C_initial)
        if contrast_gain > max_contrast:
...
def _plot_shear_averaging_box(
    ...
    * ``image_azfft[:-1] * conj(image_azfft[1:])``       — the "shear"
    ...
    if vmax <= vmin:
        vmax = vmin + 1.0
    axes[0, 0].imshow(amp, aspect="auto", cmap="gray", vmin=vmin, vmax=vmax)
...
def main() -> None:
    ...
    # Master save-verbosity switch (see `output.debug` help in the JSON config). When True every diagnostic artefact is written; when False only the two AF keeper files (`{stem}_af_before.png`, `{stem}_af_after.png`) land on disk. Compute the boolean once so the checks below stay cheap and readable.
    _save_all: bool = bool(cfg.debug)
    print(
        f"[mode] output.debug={_save_all} → "
        f"{'save-all diagnostics' if _save_all else 'minimal (af_before + af_after)'}"
    )
    ...
    # Resolve the save directory + stem once, up-front, so the six debug figures added below can save-and-close immediately as soon as each source array is available. The historical `if cfg.save is not None:` block further down reuses these values without re-probing. Falls back to /tmp on PermissionError (same rule as before).
    ...
    # --- Debug figure 1: raw input SLC |s| (rotated CW90) --- Rule "never copy s": we must NOT materialise `np.abs(s)` at full scene size (would be ~7.5 GB float32 alongside the ~15 GB complex `s`). Instead decimate to ≤ 60 M pixels first so `np.abs` runs on the strided view and produces a ~240 MB buffer; that buffer is released as soon as `_maybe_close(fig)` drops matplotlib's imshow reference. `extent` is set to the FULL scene shape so the axis labels read in original (pre-decimation) pixel units.
    if _save_all and cfg.save is not None:
        _max_display_px = 60_000_000
        _step_in = max(1, int(np.ceil(np.sqrt(s.size / _max_display_px))))
        _amp_in = np.abs(s[::_step_in, ::_step_in]).astype(np.float32, copy=False)
        _H_full_in, _W_full_in = s.shape
        _amp_in_rot = _amp_in.T[:, ::-1]
        _mu_in = float(_amp_in.mean())
        _sd_in = float(_amp_in.std())
        _vmax_in = min(float(_amp_in.max()), _mu_in + 4.0 * _sd_in)
        _vmin_in = max(0.0, _mu_in - 4.0 * _sd_in)
        fig_input_slc, _ax_in = plt.subplots(
            1, 1, figsize=(14, 16), constrained_layout=True,
        )
        fig_input_slc.suptitle(
            f"Input SLC |s|  (raw, before range degradation) — "
            f"{cfg.path.name}",
            fontsize=11,
        )
        _ax_in.imshow(
            _amp_in_rot, cmap="gray", aspect="auto",
            vmin=_vmin_in, vmax=_vmax_in,
            extent=(0.0, float(_H_full_in), float(_W_full_in), 0.0),
        )
        _ax_in.set_title(
            f"|s|: shape {_H_full_in}×{_W_full_in} (displayed decimated "
            f"×{_step_in} in each axis)"
        )
        _ax_in.set_xlabel("azimuth pixel")
        _ax_in.set_ylabel("range pixel")
        path_input_slc = save_dir / f"{stem}_input_slc.png"
        fig_input_slc.savefig(path_input_slc, dpi=350)
        print(f"Saved → {path_input_slc}")
        _maybe_close(fig_input_slc)
        del _amp_in, _amp_in_rot
    ...
    # --- Debug figure 2: |s_degraded| (post range-resolution degradation) --- Same rotated-CW90 landscape orientation as every other scene-scale PNG. `_show_slc_rot_cw90` materialises `np.abs(s_degraded)` internally (≈1 GB float32); we save-and-close immediately so the buffer is released before the sub-aperture stage allocates its own scene-scale float32 amplitudes.
    if _save_all and cfg.save is not None:
        fig_s_deg, _ax_sdeg = plt.subplots(
            1, 1, figsize=(14, 16), constrained_layout=True,
        )
        fig_s_deg.suptitle(
            f"|s_degraded|  (range coherently summed ×"
            f"{Number_of_Range_Looks}, Taylor −55 dB) — {cfg.path.name}",
            fontsize=11,
        )
        _show_slc_rot_cw90(
            _ax_sdeg, s_degraded,
            f"|s_degraded|: shape {s_degraded.shape[0]}×{s_degraded.shape[1]}",
            cmap="gray", sigma=3.0,
        )
        path_s_deg = save_dir / f"{stem}_s_degraded.png"
        fig_s_deg.savefig(path_s_deg, dpi=350)
        print(f"Saved → {path_s_deg}")
        _maybe_close(fig_s_deg)
    ...
    # --- Debug figure 3: per-Doppler-band subaperture amplitudes --- Grid of the M = N_subaperture - 2 kept sub-bands (k=0 and k=N-1 were dropped as noise inside `compute_subapertures`). Each panel is shape (sub_size, N_rg) float32, rotated CW90 for landscape orientation. Save-and-close immediately so the imshow references don't keep the ≈1.5 GB `subapertures` buffer alive after this point (the pipeline may otherwise `del subapertures` further down while the buffer is still pinned by matplotlib).
    if _save_all and cfg.save is not None:
        _n_sub_dbg = int(subapertures.shape[0])
        _n_cols_dbg = int(np.ceil(np.sqrt(_n_sub_dbg)))
        _n_rows_dbg = int(np.ceil(_n_sub_dbg / _n_cols_dbg))
        _sub_mu = float(subapertures.mean())
        _sub_sd = float(subapertures.std())
        _sub_vmin = max(0.0, _sub_mu - 2.0 * _sub_sd)
        _sub_vmax = max(_sub_mu + 2.0 * _sub_sd, _sub_vmin + 1e-12)
        fig_subaps_dbg, _axes_sd = plt.subplots(
            _n_rows_dbg, _n_cols_dbg,
            figsize=(4 * _n_cols_dbg, 3 * _n_rows_dbg),
            constrained_layout=True,
        )
        fig_subaps_dbg.suptitle(
            f"Doppler sub-aperture amplitudes — {cfg.path.name}\n"
            f"{_n_sub_dbg} kept sub-bands (edges k=0 and k=N−1 dropped), "
            f"each rotated 90° CW",
            fontsize=11,
        )
        _axes_sd_flat = np.atleast_1d(_axes_sd).ravel()
        _h_orig_sub = int(subapertures.shape[1])
        _sub_W_full = int(subapertures.shape[2])
        # Same 60 M-pixel decimation cap as `_show_slc` uses on the af_before / af_after keepers. Each rotated panel is (N_rg, sub_size) ≈ 30 M px on a full ICEYE scene — under the cap by itself, but the stride-computation is written so bigger scenes still land at a matplotlib-friendly source size.
        _sub_max_display_px = 60_000_000
        _n_pix_panel = _sub_W_full * _h_orig_sub
        if _n_pix_panel > _sub_max_display_px:
            _sub_step = int(np.ceil(np.sqrt(_n_pix_panel / _sub_max_display_px)))
        else:
            _sub_step = 1
        for _k_sd in range(_n_sub_dbg):
            _ax_sd_k = _axes_sd_flat[_k_sd]
            _view_sd = subapertures[_k_sd].T[:, ::-1]
            if _sub_step > 1:
                _view_sd = _view_sd[::_sub_step, ::_sub_step]
            _ax_sd_k.imshow(
                _view_sd, cmap="gray", aspect="auto",
                vmin=_sub_vmin, vmax=_sub_vmax,
                extent=(0.0, float(_h_orig_sub),
                        float(_sub_W_full), 0.0),
            )
            _ax_sd_k.set_title(f"sub-aperture {_k_sd}", fontsize=9)
            _ax_sd_k.set_xlabel("azimuth pixel", fontsize=8)
            _ax_sd_k.set_ylabel("range pixel", fontsize=8)
            _ax_sd_k.tick_params(labelsize=7)
        for _j_sd in range(_n_sub_dbg, _axes_sd_flat.size):
            _axes_sd_flat[_j_sd].set_visible(False)
        path_subaps_dbg = save_dir / f"{stem}_subapertures_grid.png"
        fig_subaps_dbg.savefig(path_subaps_dbg, dpi=200)
        print(f"Saved → {path_subaps_dbg}")
        _maybe_close(fig_subaps_dbg)
    ...
    # --- Debug figures 4 & 5: CoV² and CoV² + seed peaks --- CoV² = sub_var / (sub_mean² + eps) is the exact quantity `cluster_targets` gates on (see the "CoV gate" section in that function). Bright stationary clutter has CoV² near zero (stable across sub-apertures); movers spike because their brightness centroid walks look-to-look. Displayed on log10 scale so the dynamic range between sea/land clutter and mover pixels is visible. Both figures live on the decimated (sub_size, N_rg) grid so the underlying float64 buffer is small (~250 MB); save-and-close is still applied for consistency with the other debug figures. `extent = (0, W, H, 0)` follows the same convention as `_show_slc` — cols on the x-axis, rows on the y-axis — so any overlays placed in the rotated pixel frame (via `_h_orig_cov - 1 - y` on scatter x, or `_rotate_boxes_cw90(..., _h_orig_cov)` on boxes) land correctly on top of the image.
    if _save_all and cfg.save is not None and sub_mean.size:
        _cov_eps = 1e-12
        _cov_sq_dbg = sub_var / (sub_mean ** 2 + _cov_eps)
        _cov_log = np.log10(_cov_sq_dbg + _cov_eps)
        _cov_vmin = float(np.percentile(_cov_log, 2.0))
        _cov_vmax = float(np.percentile(_cov_log, 99.5))
        _h_orig_cov = int(_cov_log.shape[0])
        _cov_rot = _cov_log.T[:, ::-1]
        _cov_H, _cov_W = _cov_rot.shape
        _cov_extent = (0.0, float(_cov_W), float(_cov_H), 0.0)

        fig_cov, _ax_cov = plt.subplots(
            1, 1, figsize=(14, 16), constrained_layout=True,
        )
        fig_cov.suptitle(
            f"log10 CoV²  =  sub_var / sub_mean²  (Doppler sub-aperture "
            f"scintillation gate) — {cfg.path.name}",
            fontsize=11,
        )
        _ax_cov.imshow(
            _cov_rot, cmap="gray", aspect="auto",
            vmin=_cov_vmin, vmax=_cov_vmax, extent=_cov_extent,
        )
        _ax_cov.set_title(
            f"log10 CoV² on decimated grid "
            f"{sub_mean.shape[0]}×{sub_mean.shape[1]}  "
            f"(threshold used by cluster_targets: log10(th) = "
            f"{np.log10(cluster_result.threshold + _cov_eps):.2f})"
        )
        _ax_cov.set_xlabel(
            f"azimuth pixel (decimated, ×{N_subaperture} = s_degraded row)"
        )
        _ax_cov.set_ylabel("range pixel (decimated)")
        path_cov = save_dir / f"{stem}_subaperture_cov.png"
        fig_cov.savefig(path_cov, dpi=350)
        print(f"Saved → {path_cov}")
        _maybe_close(fig_cov)

        # CoV² again, this time with just the seed peaks (red ×) overlaid — no cluster bounding boxes on this panel (the box overlays live on `{stem}_sub_mean_boxes.png` and `{stem}_s_degraded_detections.png` further down; keeping this panel peaks-only makes the scintillation gate's raw output easier to read against the CoV² background).
        fig_cov_det, _ax_cov_det = plt.subplots(
            1, 1, figsize=(14, 16), constrained_layout=True,
        )
        fig_cov_det.suptitle(
            f"log10 CoV² + seed peaks — {cfg.path.name}",
            fontsize=11,
        )
        _ax_cov_det.imshow(
            _cov_rot, cmap="gray", aspect="auto",
        ...
        if len(peaks_yx):
            _ax_cov_det.scatter(
                _h_orig_cov - 1 - peaks_yx[:, 0], peaks_yx[:, 1],
                s=10, marker="x", c="red", linewidths=0.6,
                label=f"seed peaks ({len(peaks_yx)})",
            )
            _ax_cov_det.legend(loc="upper right", fontsize=8)
        _ax_cov_det.set_title(
            f"log10 CoV² + {len(peaks_yx)} seed peaks (red ×)"
        )
        _ax_cov_det.set_xlabel(
            f"azimuth pixel (decimated, ×{N_subaperture} = s_degraded row)"
        )
        _ax_cov_det.set_ylabel("range pixel (decimated)")
        path_cov_det = save_dir / f"{stem}_subaperture_cov_detections.png"
        fig_cov_det.savefig(path_cov_det, dpi=350)
        print(f"Saved → {path_cov_det}")
        _maybe_close(fig_cov_det)
        del _cov_sq_dbg, _cov_log, _cov_rot
    ...
    if _save_all and cfg.save is not None and sub_mean.size:
        ...
        _im = _ax_sub_mean.imshow(
            _sub_mean_rot, cmap="gray", aspect="auto",
    ...
    # --- Debug figure 6: |s_degraded| + initial cluster_targets detections --- Every box that came out of the detection stage (i.e. `boxes_yxhw_initial`, before ANY phase / motion / quality filter has run), overlaid on `|s_degraded|` in the standard rotated-CW90 orientation. Complements `{stem}_kept_vs_eliminated.png` which is written later once the full filter chain has decided; this figure shows the raw detection output on its own so the filter behaviour downstream is directly attributable. Save-and-close immediately so the ≈1 GB `np.abs(s_degraded)` buffer that imshow holds is released before the phase-filter chain builds its own scene-scale overlay figures.
    if _save_all and cfg.save is not None:
        fig_s_deg_det, _ax_sdd = plt.subplots(
            1, 1, figsize=(14, 16), constrained_layout=True,
        )
        fig_s_deg_det.suptitle(
            f"|s_degraded| + initial cluster_targets detections — "
            f"{cfg.path.name}",
            fontsize=11,
        )
        _, _, _h_orig_sdd = _show_slc_rot_cw90(
            _ax_sdd, s_degraded,
            f"|s_degraded|: {len(boxes_yxhw_initial)} raw cluster_targets "
            f"boxes (cyan, before any filter)",
            cmap="gray", sigma=3.0,
        )
        if len(boxes_yxhw_initial):
            _overlay_boxes(
                _ax_sdd,
                _rotate_boxes_cw90(boxes_yxhw_initial, _h_orig_sdd),
                color="cyan", lw=1.0,
            )
        path_s_deg_det = save_dir / f"{stem}_s_degraded_detections.png"
        fig_s_deg_det.savefig(path_s_deg_det, dpi=350)
        print(f"Saved → {path_s_deg_det}")
        _maybe_close(fig_s_deg_det)
    ...
    # The set difference is computed on integer (y_c, x_c, h, w) tuples: every filter only index-selects rows out of `boxes_yxhw_initial`, never mutates them, so a survivor row appears verbatim in `boxes_yxhw`. Full-pipeline audit of which detections the phase / motion / contrast gates threw away. Saved further down as `{stem}_kept_vs_eliminated.png` only when `cfg.debug` is on.
    if _save_all and cfg.save is not None:
        if len(boxes_yxhw_initial):
            _kept_set = {
    ...
    if _save_all and cfg.display_all_mode:
        ...
        for k in range(n_sub):
            ax_k = axes3_flat[k]
            ax_k.imshow(
                subapertures[k], cmap="gray", aspect="auto",
    ...
    if cfg.save is not None:
        # `save_dir`, `stem`, `path_main`, `path_csv` and `per_box_dir` were resolved once at the top of the pipeline (right after the SLC load) so the early debug figures (input SLC / s_degraded / sub-apertures / CoV² / CoV² + detections / s_degraded + detections) could save-and-close as they were built. Reuse those values here without re-probing.

        # Save the main overview figure + the other intermediate diagnostic figures. Written only when `cfg.debug=True`; in minimal mode only `{stem}_af_before.png` / `{stem}_af_after.png` survive.
        if _save_all:
            if fig is not None:
        ...
        if K:
            ...
            # Per-box 2D d_phase image (companion to `box_NNN_dphase.png`). Shows the RAW 2D azimuth phase derivative arg{s_degraded[u+1,v]·s*_degraded[u,v]} inside the same crop that `compute_box_phase_estimates` used — the pre-collapse view before the amplitude-weighted circular mean across range collapses it to phi(i). Rotated CW90 to landscape. Reuses `box_y0[k]` / `box_n_rows[k]` from `compute_box_phase_estimates` so the crop is bit-for-bit identical to the one that produced the trace figure.
            n_rg_sd = int(s_degraded.shape[1])
            n_dphi_written = 0
            for k, (_y_c, x_c, _h, w) in (
                enumerate(boxes_yxhw) if _save_all else ()
            ):
                y0_k = int(box_y0[k])
                L_k = int(box_n_rows[k])
                if L_k < 2:
                    continue
                y1_k = y0_k + L_k
                x0_k = max(0, int(round(float(x_c) - float(w) / 2.0)))
                x1_k = min(n_rg_sd, int(round(float(x_c) + float(w) / 2.0)))
                if x1_k - x0_k < 1:
                    continue
                # Per-box crop small (typically ~1e4 to ~1e5 pixels) — the transient complex multiply here is negligible and never approaches `s_degraded.size`.
                s_box_k = s_degraded[y0_k:y1_k + 1, x0_k:x1_k]
                d_phi_box = np.angle(
                    s_box_k[1:] * np.conj(s_box_k[:-1])
                ).astype(np.float32, copy=False)
                d_phi_box_rot = d_phi_box.T[:, ::-1]

                fig_di, ax_di = plt.subplots(
                    figsize=(12, 5), constrained_layout=True,
                )
                star_di = "*" if bool(strong_mask[k]) else " "
                ax_di.imshow(
                    d_phi_box_rot, cmap="twilight", aspect="auto",
                    vmin=-np.pi, vmax=np.pi,
                    extent=(float(y0_k), float(y1_k),
                            float(x1_k), float(x0_k)),
                )
                slope_k_pr = float(box_slope_rad_per_row[k])
                slope_str_di = (
                    f"  slope={np.degrees(slope_k_pr):+.2f}°/row"
                    if np.isfinite(slope_k_pr) else "  (fit failed)"
                )
                ax_di.set_title(
                    f"{star_di}box {k:02d}  raw d_phase image  "
                    f"y=[{y0_k}, {y1_k}]  x=[{x0_k}, {x1_k}]  "
                    f"shape ({L_k}, {x1_k - x0_k}){slope_str_di}  "
                    r"(wrapped in $(-\pi, \pi]$)",
                    fontsize=9,
                )
                ax_di.set_xlabel("absolute az row (on s_degraded)")
                ax_di.set_ylabel("range col (on s_degraded)")

                path_box_dphi_img = (
                    per_box_dir / f"box_{k:03d}_dphase_image.png"
                )
                fig_di.savefig(path_box_dphi_img, dpi=130)
                _maybe_close(fig_di)
                n_dphi_written += 1
            if _save_all and n_dphi_written:
                print(
                    f"Saved {n_dphi_written} per-box d_phase image PNGs → "
                    f"{per_box_dir}/box_000_dphase_image.png … "
                    f"box_{K-1:03d}_dphase_image.png"
                )
            ...
            # Building + saving the two full-image overview PNGs below ({stem}_af_before.png, {stem}_af_after.png) is excluded from the timing report — they are large-canvas plot/save operations dominated by matplotlib + PNG encoding cost, not pipeline work we care about benchmarking.
            sw.pause()
            ...
            # `{stem}_af_boxes.png` (pre-AF-paste overview with bounding rectangles) was retired: the `af_before` keeper already shows the same boxes on the same amplitude scene, at a much higher DPI.
            ...
            # `{stem}_af_corrected.png` (post-AF-paste overview at dpi=150) was retired: the `af_after` keeper already renders the same mutated `s` with the same rectangles, at a much higher DPI.
